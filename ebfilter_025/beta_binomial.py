...
def beta_binomial_loglikelihood(params, Ns, Ks):
    ...
    Args:
        ...
        Ks (list([int])): the counts for success
        
        Ns (list([int])): the counts of trials
    ...
    ML = ML + sum([math.lgamma(n + 1)       for n   in Ns           ])
    ML = ML - sum([math.lgamma(k + 1)       for k   in Ks           ])
    ML = ML - sum([math.lgamma(n - k + 1)   for n,k in zip(Ns,Ks)   ])
    
    ML = ML - sum([math.lgamma(n + alpha + beta)    for n   in Ns           ])
    ML = ML + sum([math.lgamma(k + alpha)           for k   in Ks           ])
    ML = ML + sum([math.lgamma(n - k + beta)        for n,k in zip(Ns,Ks)   ])
...
def fit_beta_binomial(As, Bs, epsilon):
